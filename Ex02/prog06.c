...
int main()
{
  ...
  FILE *fp;
  /* file open & check */
  fp = fopen("score_data","r");
  if(fp == NULL)
    {
      printf("Cannot open file score_data\n");
      exit(8);
    }

  /* ファイルから学籍番号と点数を読み取るコードをここに書く。 */
  for(i = 0; i < MAX_ST_NUM; i++)
    {
      status = fscanf(fp,"%d %d",&student_id[i] ,&abs_score[i]);
      //学生番号と補正前点数を読み込んだ
      if(status != 2) break;
      num_student++;
    }
  ...
  while(1)
    {
      ...
      status = scanf("%c",&grade);
      if(status == EOF) break;
      
      grade_search(grade);
      
      scanf("%c",&grade);//改行文字を省いた
    }
  printf("\n");
